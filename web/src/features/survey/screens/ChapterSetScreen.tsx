...
import {
  WorkloadSvg, RelationsSvg, LeaderSvg, GrowthSvg,
  PersonalSvg, ToolsSvg, AltroSvg,
} from './CausaIcons'
...
const GROUP_ICON: Record<string, React.FC> = {
  'I colleghi':               RelationsSvg,
  'Responsabile':             LeaderSvg,
  'HR':                       PersonalSvg,
  'Management':               LeaderSvg,
  'Autonomia nel lavoro':     ToolsSvg,
  'Crescita professionale':   GrowthSvg,
  'Valori aziendali':         PersonalSvg,
  'Tecnologia':               ToolsSvg,
  'Carico di lavoro':         WorkloadSvg,
  'Recupero':                 PersonalSvg,
  'Cosa vorresti migliorare': AltroSvg,
  'Open Listening 2025':      RelationsSvg,
  'Passione':                 PersonalSvg,
  'Raccomandazione':          GrowthSvg,
}

function GroupIcon({ group }: { group: string }) {
  const clean = stripEmoji(group)
  const Icon = GROUP_ICON[clean]
  if (!Icon) return null
  return (
    <span className="cs-group-icon" aria-hidden>
      <Icon />
    </span>
  )
}
